...
const Checkout = () => {
  ...
  //getAddresses(token).then((data) => { console.log(data,"hahah")});
  /**
  ...
  const addAddress = async (token, newAddress) => {
    console.log(token,"toookkkkeeen")
    try {
  ...
  const deleteAddress = async (token, addressId) => {
    console.log(token , "hbhjhpiucvtgyscfgcvtex")
    try {
     ...
     const response=await axios.delete(`${config.endpoint}/user/addresses/${addressId}`,
     {
      ...
      console.log(e.response.data.message,"hahah");
      if (e.response) {
        enqueueSnackbar(e.response.data.message, { variant: "error" });
      } else {
        enqueueSnackbar(
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try{
      ...
      if(e.response){
        ...
        enqueueSnackbar("could not place order,check that the backenf is ruuning, reachable and return valid JSON",{variant:"error"});
      }
    }
  };
  ...
  // TODO: CRIO_TASK_MODULE_CHECKOUT - Fetch addressses if logged in, otherwise show info message and redirect to Products page
   useEffect(()=>{
    if(token){
      ...
      enqueueSnackbar("You must be login in to access checkout page",{variant:"info"});
      history.push("/");
    }
   },[token]);
